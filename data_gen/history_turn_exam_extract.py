...
def generate_histories(inputs, output_file, turn_id, max_tokens=500):
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile:
        for i, input in enumerate(inputs):
            ...
            if "remove" in input and input["remove"] == "yes":
                removed += 1
            if input["generation_finished"] == "yes" or input[f"dialogue_t{turn_id}"]["check_extracted"]["pass_exam"] == "yes":
                ...
                output_data[f"dialogue_t{turn_id}"]["check_extracted"]["pass_exam"] = answer
    ...
    print(f"Passed: {passed}/{len(inputs)}, Removed: {removed}")
...
if __name__ == "__main__":
    ...
    input_dir = f"testset\\output\\{task}\\history\\turn_{args.turn_id}\\dialogue_i-{args.iter_id}_examed.jsonl"
    output_dir = f"testset\\output\\{task}\\history\\turn_{args.turn_id}\\dialogue_i-{args.iter_id}_examed_extracted.jsonl"
